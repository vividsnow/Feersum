...
# synthetic code, so accepting it let a server that never answers count as
# "rejected".  Verified on Linux.  On the BSDs the reject is observed as a
# connection error instead of a delivered response, and no BSD box was
# available to establish whether that is the socket layer or Feersum, so the
# synthetic code is still tolerated there rather than asserted away.
