...
# ========================================
{
    ...
    # Test: PROXY v1 terminated with a bare LF instead of CRLF.  The scan for
    # the terminator used to run straight past it to the next CRLF, so the
    # header still "parsed" - just ending somewhere the sender never intended.
    # The trailing "X\r\n" here is what makes that visible: it used to be
    # absorbed into the header, leaving a complete, valid request behind it
    # and a 200.  Asserting only 4xx would not have caught it, because a bare
    # LF with nothing after it also yields 4xx on the broken build - the
    # request line gets eaten and what remains is merely malformed.
    {
