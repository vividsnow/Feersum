...
# Test 5: set_tls with listener index out of range
{
    ...
    # Indices past 32 bits were narrowed to int BEFORE the range check, so
    # 2**32 wrapped to 0 and silently configured TLS on a listener the caller
    # never named.  Both of these must be refused, not quietly redirected.
    for my $huge (4294967296, 4294967297) {
