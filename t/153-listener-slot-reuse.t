...
#
# Two ways in.  A croak out of setup_accept_watcher (epoll_create1 failing
# under fd pressure) leaves the slot committed with its fd already assigned, so
# retrying use_socket duplicates it - that one needs fd exhaustion to provoke.
# The other is just calling use_socket twice with one socket, which is what
# this test does because it needs no special conditions.
use warnings;
use strict;
