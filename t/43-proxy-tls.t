...
# ========================================
{
    ...
    if ($pid5 == 0) {
        ...
        IO::Socket::SSL->start_SSL($raw,
            ...
            # A connection dropped mid-handshake is a load symptom: on a starved
            # smoker the server can time out the slow byte-by-byte PROXY header
            # and close before the ClientHello arrives.  Signal that distinctly
            # (exit 4) so the parent skips instead of failing; a genuine TLS
            # negotiation failure still exits 2 and fails.  "SSL connect attempt
            # failed" with nothing further is how LibreSSL on macOS words the
            # same drop that OpenSSL reports as eof/syscall.
