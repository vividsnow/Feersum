...
SKIP: {
    # 4 implicit simple_client passes + 1 count + 4 status + 1 shutdown = 10
    skip "EPOLLEXCLUSIVE not available (requires Linux 4.5+)", 10
        unless $has_epoll_exclusive;
    ...
    # Cleanup.  "cleanly" has to be checked: the wait status was discarded, so
    # a master that dumped core on shutdown still passed.
    kill 3, $pid;  # QUIT
    waitpid $pid, 0;
    is $? & 127, 0, "server shut down cleanly with EPOLLEXCLUSIVE (no signal)";
}
