...
# Check if IPv6 is available and can bind (must be in BEGIN for plan)
my ($has_ipv6, $ipv6_bind_works);
BEGIN {
    eval {
        require Socket;
        ...
        $has_ipv6 = defined $addr;
        if ($has_ipv6) {
            # Try to actually bind to ::1 to see if it works
            my $sock;
            socket($sock, AF_INET6(), SOCK_STREAM(), 0) or die "socket: $!";
            bind($sock, pack_sockaddr_in6(0, $addr)) or die "bind: $!";
            close($sock);
            $ipv6_bind_works = 1;
        }
    };
...
plan tests => $ipv6_bind_works ? 19 : 13;
...
SKIP: {
    skip "IPv6 bind to ::1 not available on this system", 6 unless $ipv6_bind_works;
