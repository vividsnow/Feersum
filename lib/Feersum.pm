...
upgrade), where each socket read invokes it.  C<psgix.input.buffered> stays
C<1> either way.
...
silence in B<either> direction: a client that stops sending WINDOW_UPDATEs
mid-download, and so stops sending anything at all, is reaped after this long
even though a response is still in flight.  Real clients acknowledge data as
they consume it, so this reaps a stalled peer rather than a slow one.
...
Only effective on Linux 4.5+.  Enabling it in a build whose headers lack
C<EPOLLEXCLUSIVE> (any non-Linux platform, or pre-4.5 build headers) emits a
warning and has no effect.  A build that has the flag but runs on a kernel too
old to honour it fails later instead, when the listener's accept watcher is
created.  Disabling it (passing a false value) is always silent.
...
400 response, HTTP/2 a stream reset.
...
Upgrading from 1.505 is a large jump (1.506 covered part of it).  Most changes are
...
large uploads or long query strings; see L</METHODS>.
