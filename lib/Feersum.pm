...
HEAD requests get no special treatment: the handler (or middleware such as
L<Plack::Middleware::Head>) is responsible for replying to HEAD with an
empty body.
...
(e.g. GET), C<< read($buf, $env->{CONTENT_LENGTH}) >> returns 0 (CONTENT_LENGTH
is 0); a read requesting a positive length returns C<undef> with C<$!> set to
C<EAGAIN>, since no body data is available.
...
C<FD_CLOEXEC> set and (for TCP) C<TCP_NODELAY> enabled.  For TLS and HTTP/2 connections, C<psgix.io>
returns a Unix socketpair that relays data through the TLS/H2 layer
transparently.
...
streams); see L<Feersum::Connection::Handle/sendfile>.
...
Only effective on Linux 4.5+.  Enabling it on other platforms (or older
Linux) emits a warning and has no effect; disabling it (passing a false
value) is always silent.

Set this B<before> C<use_socket()>/C<accept_on_fd()>: the flag is read when
each listener's accept watcher is created, so it only takes effect for
listeners added afterwards.
...
=item C<resp_done(fd, code)>

Fired when a response has finished being written.  The C<code> argument is
reserved for future use (always 0).

=item C<trace(level, message)>

Fired for diagnostic trace messages when Feersum is built with tracing.
Only emitted when the corresponding USDT probe is enabled by a consumer.
