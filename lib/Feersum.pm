...
any body the handler supplies is discarded.  A C<HEAD> response is sent
...
Like C<use_socket>, with a file descriptor number.  The descriptor must be
non-blocking: Feersum accepts several connections per loop iteration, and a
blocking C<accept()> wedges the whole loop.
...
HTTP/2 via ALPN (otherwise only C<http/1.1> is offered).  It is
...
not run: HTTP/1.1 gets 431, HTTP/2 a stream reset.

=item MAX_HEADER_NAME_LEN

Default 128.  Maximum header name length.  Over the limit: 431 on HTTP/1.1,
a stream reset on HTTP/2.
...
to completion when the handler returns, flushing incrementally, but the
drain is not event-loop-paced: it does not read the socket while running, so
a large or slow body occupies the worker until fully read.  Prefer the
streaming writer API for such responses.
...
faster.  TLS 1.3 via picotls costs about 18% on the native interface and
under 10% on PSGI; expect more without AES-NI.
...
served over HTTP/1.1 with default settings needs no action.
...
C<Feersum::Runner>'s C<user> option now also derives the group from the
user's passwd entry, clears supplementary groups, and verifies the drop (it
used to call only C<setuid>, keeping root's groups).  A configuration that
cannot complete the group drop croaks.
...
fatal C<no_application_protocol> alert instead of an HTTP/1.1 fallback.
Clients that send no ALPN extension are unaffected.
...
C<Content-Length>, and 205 is now a no-body status.
...
=item * A streaming response honours an application-supplied
C<Content-Length> instead of discarding it; a wrong one truncates or
over-runs the response.

=item * A response header name or value containing CR or LF is rejected with
500 (response splitting).  Packing two C<Set-Cookie> values into one string
no longer works.
...
=item * Duplicate request headers are joined (with C<", ">, or C<"; "> for
C<Cookie>) instead of only the first being reported.
...
=item * C<return_from_psgix_io()> and the native C<return_from_io()> croak
unless handed the handle that wraps this connection's socket.

=item * Handing the socket back takes a private duplicate of the descriptor,
so the returned handle can simply be released when done.
...
install your own handler (L</DIED>) to change that.
...
=item * C<SIGTERM> triggers a graceful shutdown: exit 0, up to
C<graceful_timeout> later.
