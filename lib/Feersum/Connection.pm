...
Input body handler (psgi.input).  Returns C<undef> for requests with no body
(e.g., GET, HEAD) -- check the return value before calling C<read()>.  It is
advised to close it after read is done.
