...
https://www.varnish-cache.org/trac/ticket/400
...
Handing the socket back ends the original request: C<env()> and C<method()>
croak afterwards, and a response sent from here has no request to be shaped
by.  That matters for one case - answering a C<HEAD> after the hand-back
sends the body a HEAD must not have - so decide before calling C<io()>
whether the request can carry one.
...
tunnel C<io()> works but the tunnel cannot be handed back; on a TLS tunnel
the socketpair relay cannot be returned.)
