...
BEGIN {
    # IPv6 support (Socket 1.95+, Perl 5.14+).  Socket exports the constant
    # even where its build found no macro for it and croaks only when called,
    # so the probe has to call it.
...
sub _create_socket { ## no critic (ProhibitExcessComplexity)
    ...
    if (_is_unix_listen($listen)) {
        ...
        # pack_sockaddr_un only WARNS before truncating, so an over-long path
        # bound a prefix of itself and every later start then failed to bind.
...
# File-scoped: the server drops the retirement callback once it has run, and
# a watcher held only in that closure's pad dies with it.  A process retires once.
my $RETIRE_DEADLINE;
...
sub _set_max_requests_per_worker {
    ...
    $f->max_requests_per_worker($max,
        ...
        sub {
            # Nothing else bounds this drain, and the replacement is forked
            # only once we exit - at pre_fork => 1 that is a total outage.
...
sub _is_unix_listen {
    my ($listen) = @_;
    # Anything that starts like a path.  The trailing \w this used to require
    # disagreed with _normalize_listen's own test, so a spec such as
    # '/-foo.sock' skipped the "no port" check as a path and was then bound as
    # a TCP address, dying with "Invalid argument".
...
my $SO_REUSEPORT_WORKS;   # probed once: a kernel property, not a per-server one
...
sub _reuseport_enabled {
    ...
    # The constant only says the headers had it: pre-3.9 kernels export it
    # and then fail setsockopt with ENOPROTOOPT.
...
sub _prepare { ## no critic (ProhibitExcessComplexity)
    ...
    # accept_priority must be set BEFORE use_socket(): each accept watcher
...
sub run { ## no critic (ProhibitExcessComplexity)
    ...
    # A daemonize+pre_fork pool that never came up is a failed start: _exit
    # without the ready token so the parent sees EOF and skips the pid file,
    # rather than backgrounding a dead pool as a clean start.
...
# Hot restart master: binds the sockets once, then manages generations, each a
# forked child running _apply_settings + app load + serve.  SIGHUP forks a new
# generation and SIGQUITs the old one once it is ready.
# Called BEFORE _daemonize_and_write_pid so a bad port or listen address is
# discovered while the exit status can still carry it - after daemonizing,
# "port already in use" exits 0 and a Type=forking supervisor calls that a
# clean start.  Idempotent, so the non-daemonized path can call it too.
...
sub _run_hot_restart_master { ## no critic (ProhibitExcessComplexity)
    ...
    my $gen_ready = 0;
    # Backoff state for restarting a crashed generation (see the reaper below).
    my ($gen_ready_at, $gen_restart_fails, $gen_restart_timer);

    # Master liveness pipe: the master is the sole holder of the write end, so
    # every generation sees EOF the instant the master dies (crash, SIGKILL,
    # OOM) and drains, instead of orphaning itself and its workers still holding
    # the listen socket - which would fail a replacement master's bind.
    ...
    my $fork_generation = sub {
        ...
        if ($pid == 0) {
            ...
            my $f = Feersum->endjinn;
            # accept_priority before use_socket (see _prepare); the XS
            ...
            # HUP is the master's reload signal; delivered to the generation (a
            # process-group HUP, or a mis-aimed kill) it would otherwise
            # terminate it by default disposition.  Ignore it, as the cold-start
            # supervisor does.
            ...
            # Drain if the master dies (nothing left to reap us, and we hold the
            # listen socket a replacement needs).  Close our write-end copy so
            # only the master holds one, then watch for its EOF - self-clearing,
            # as a level-triggered EV::io on a dead pipe would busy-spin.
            ...
            if ($self->{pre_fork}) {
                ...
                # Gate readiness on workers that actually came up.  A reload
                # whose workers die at load fails here - the master keeps the
                # old generation - instead of reporting ready and retiring it.
            ...
            # Signal master: ready to serve (workers are up, not just forked).
    ...
    # Restart the active generation after an unexpected death.  Called inline by
    # the reaper for a long-lived death, or from a backoff timer for one that
    # died right after coming up - otherwise a crash-after-ready app spins the
    # master re-forking generations as fast as it can load the app.
    ...
    $reap = EV::child 0, 0, sub {
        ...
        if ($current_pid && $kid == $current_pid) {
            ...
            unless ($shutting_down || $pending_pid) {
                # A generation that dies right after coming up is crash-looping;
                # back off before re-forking rather than spin.  A long-lived one
                # restarts at once.
...
# Wait until every pre_fork worker has reported readiness (app loaded +
# after_fork run) or the startup deadline passes.  The reaper runs in this
# loop, so a slot that dies before reporting is respawned and can still make
# it.  Returns the number of distinct slots that came up.
sub _wait_for_workers_ready {
    ...
    my $rfh = $pipe->[0];
    my $expired = 0;
    # Our write end stays open so a slot respawned here inherits it; gate on the
    # count and the deadline rather than EOF.  One byte per worker: a slot that
    # dies and respawns before reporting still sends exactly one once it is up,
    # and a traffic-less startup offers no retirement to double-count.
...
# Drop our copy of the write end so the parent is the only holder, then drain
# when it EOFs.  A signalled shutdown always gets there first.
...
sub _fork_another { ## no critic (ProhibitExcessComplexity)
    ...
    # One pipe for the pool: only this process keeps the write end, so a
    # worker sees EOF however its parent died - SIGKILL, crash, KILL backstop.
    ...
    unless ($pid) {
        ...
        if ($self->{_use_reuseport}) {
            ...
            # Guard like the loader below: an unguarded croak here (e.g. a
            # root-owned key unreadable after privdrop) unwinds the fork child
            # into inherited supervisor frames, spawning plaintext-serving
            # clones on the TLS listener it already made accepting.  _exit instead.
            ...
            # This worker owns its TCP listen sockets, so connections the
            # kernel queued on them die with the fd at shutdown (client gets
            # RST).  Accept and serve them as part of the graceful drain;
            # inherited (shared) sockets keep the default off so a live
            # sibling takes the queue instead.
        ...
        # Report readiness so the parent can gate its own - USR2 to the
        # hot_restart master, or the daemonize success token - on workers that
        # loaded the app and ran after_fork, not merely forked.  One byte each;
        # the parent counts them.
        if (my $rp = $self->{_worker_ready_pipe}) {
...
sub _start_pre_fork {
    ...
    # Under daemonize, gate the ready token on workers actually coming up: a
    # dead pool then fails the start (parent sees EOF, skips the pid file)
    # instead of being backgrounded.  Foreground keeps the respawn loop as is.
...
sub _daemon_ready {
    ...
    # The parent may already have timed out and closed its end (a slow start is
    # reported as success); a bare write would then take SIGPIPE.
    local $SIG{PIPE} = 'IGNORE';
    syswrite $fh, 'R';
    close $fh;
    return;
...
# Check writability BEFORE daemonizing, or the daemon is already serving when
# the parent croaks.  Records whether the probe created the file, so a failed
# start cleans up without removing one that may belong to a running server.
...
# Undo the probe above when the start it was probing for never happened.  Left
# behind, a zero-byte pid file still satisfies `test -f`, so a failed start
# reads to a health check as a successful one - and the next `kill $(cat ...)`
# gets an empty pid.
...
sub _drop_privs { ## no critic (ProhibitExcessComplexity)
    ...
    if (defined $gid) {
        ...
        # setgroups needs CAP_SETGID, so only root can shed them; a non-root
        # start naming the user it already is has none to shed and would only
        # EPERM here, so skip it in that case.
        my $root = ($> == 0);
        if ($root) {
        ...
        # Verify the drop took effect.  With setgroups (root) the whole GID list
        # must read back as "gid gid" - real gid plus the one-entry setgroups
        # list, since $) = "g g" sets setgroups([g]), not an empty list - so
        # every token must equal $gid.  Without it, only the real GID (the first
        # token) was ours to set.
        my @rg = split ' ', $(;
...
give more fair distribution across prefork workers sharing a listen socket.
...
B<Worker churn.>  A reuseport socket owns its own accept queue, so when a
worker stops accepting (C<max_requests_per_worker> retirement, C<hot_restart>
reload, shutdown), whatever the kernel had already queued on that socket
would be destroyed rather than redistributed - those clients completed their
TCP handshake and would see a connection reset having sent nothing.  Every
reuseport worker therefore runs with C<set_drain_accept_queue> enabled (see
L<Feersum>): before closing its listener it accepts what the kernel already
queued and serves those connections as part of the graceful drain, and
C<TCP_DEFER_ACCEPT> stays off so no established connection is invisible to
that drain.  Measured with four parallel clients hammering C<pre_fork
=E<gt> 2> workers retiring every 30 requests, destroyed-connection resets
went from 1.6% of connections to 0.02%; without C<reuseport> the listen
socket outlives any one worker and the same churn loses nothing.

What churn still costs with C<reuseport> is connect-time failures, not
destroyed requests: between a worker closing its listener and its
replacement binding, the reuseport group is one member short, and if every
member is in that gap at once a new SYN has nowhere to land and is refused;
separately, a handshake whose final ACK is still in flight when the listener
closes is reset before the drain can see it.  Both need pathological churn
to matter - workers retiring many times a second - and shrink linearly with
the retirement interval and sharply with worker count (measured at
retirement every ~10ms: 2.9% refused at C<pre_fork =E<gt> 2>, 0.9% at 3; at
a realistic C<max_requests_per_worker> in the thousands both are
negligible).  Linux 5.14+ can close both entirely at the kernel level with
C<net.ipv4.tcp_migrate_req=1>, which migrates queued and half-open
connections to a surviving group member instead of resetting them.
...
C<graceful_timeout> bounds the retiring worker's drain, exactly as it bounds a
shutdown.  It has to: the replacement is forked only once the retiring worker
exits, so an unbounded drain leaves the slot empty for as long as one slow
connection cares to hold it.
...
have a chance to drain and exit first.  It bounds a C<max_requests_per_worker>
...
B<Note:> the TLS certificate and key must be readable by C<user>, because a
worker respawn (a crash, or the routine C<max_requests_per_worker> retirement)
re-reads them after the drop.  A key readable only by root (the usual
C<< /etc/ssl/private >> layout) B<works at startup> -- the first workers
inherit the context loaded as root -- but then quietly empties the pool at the
first respawn, and under C<daemonize> the diagnostic is lost.  Copy the key
where C<user> can read it, or grant access through its group.
