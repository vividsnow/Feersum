    ...
    if ($listen =~ m#^[/\.]+\w#) {
        ...
        if ($use_reuseport && defined SO_REUSEPORT) {
            ...
            if ($listen =~ /^\[([^\]]+)\]:(\d*)$/) {
                ...
                # Bare IPv6 (multiple colons): ::1 or 2001:db8::1
...
sub _fork_another {
    ...
    unless ($pid) {
        ...
        if ($self->{_use_reuseport}) {
            ...
            close($self->{sock}) or carp "close socket: $!" if $self->{sock};
    ...
    $self->{_kids}[$slot] = EV::child $pid, 0, sub {
        ...
        unless ($self->{_use_reuseport}) {
            ...
            if (defined $fd) {
                ...
                carp "fileno returned undef during respawn";
            }
        }
        else {
...
sub _start_pre_fork {
    ...
    # Validate pre_fork value
    my $n = $self->{pre_fork};
    if (!defined $n || $n !~ /^\d+$/ || $n < 1) {
        croak "pre_fork must be a positive integer";
    }
    if ($n > MAX_PRE_FORK) {
        carp "pre_fork=$n exceeds " . MAX_PRE_FORK . "; consider a lower value";
    }

    POSIX::setsid() or carp "setsid() failed: $!";
