...
sub _prepare {
    ...
    if (my $opts = $self->{options}) {
        $self->{$_} = delete $opts->{$_} for grep defined($opts->{$_}),
            qw/pre_fork keepalive read_timeout header_timeout write_timeout max_connection_reqs reuseport epoll_exclusive
               ...
               reverse_proxy proxy_protocol psgix_io h2 tls tls_cert_file tls_key_file/;
    ...
    if (my $tls = delete $self->{tls}) {
        ...
        if ($f->has_tls()) {
            ...
            # Save TLS config for reuseport children to re-apply
...
sub _fork_another {
    ...
    unless ($pid) {
        ...
        if ($self->{_use_reuseport}) {
            ...
            # Re-apply TLS config on new listeners
            if (my $tls = $self->{_tls_config}) {
                for my $i (0 .. $#new_socks) {
                    $self->{endjinn}->set_tls(listener => $i, %$tls);
                }
            }
        }
