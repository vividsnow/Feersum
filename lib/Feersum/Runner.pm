...
# Walk @SIMPLE_SETTINGS plus the can()-gated extras. $consume=1 deletes from
# $self after applying (cold-start path); $consume=0 leaves $self intact
# (hot_restart child re-applying preserved config from master).
sub _apply_simple_settings {
    ...
    if ($f->can('max_h2_concurrent_streams')) {
        my $v = $consume ? delete $self->{max_h2_concurrent_streams}
                         : $self->{max_h2_concurrent_streams};
        $f->max_h2_concurrent_streams($v) if defined $v;
    }
}

# Install the periodic watcher that triggers graceful shutdown once the
# worker has served max_requests_per_worker requests. Returns the watcher
# SV; caller must keep the reference alive (a my-scoped lexical works).
# Used by both the non-prefork hot_restart generation and pre_fork workers.
sub _install_max_requests_watcher {
    my ($self, $f) = @_;
    my $max = $self->{max_requests_per_worker} or return;
    my $w; $w = EV::timer(1, 1, sub {
...
sub _prepare {
    ...
    if (defined $self->{accept_priority} && $f->can('accept_priority')) {
        my $val = $self->{accept_priority};
        croak "accept_priority must be an integer" unless $val =~ /^-?\d+$/;
        croak "accept_priority must be between -2 and 2" if $val < -2 || $val > 2;
    ...
    # Validate priorities (-2..+2 per libev) before applying
    ...
    my $fork_generation = sub {
        ...
        if ($pid == 0) {
            ...
            if ($self->{pre_fork}) {
                ...
                POSIX::setsid() or warn "Feersum [$$]: setsid failed: $!\n";
                $self->{_kids} = [];
                $self->{_n_kids} = 0;
            ...
            if (!$self->{pre_fork}) {
                ...
                $mrw = $self->_install_max_requests_watcher($f);
            }
...
sub _apply_settings {
    ...
    # use_socket; this keeps the two settings paths consistent).
...
sub _fork_another {
    ...
    unless ($pid) {
        ...
        my $max_reqs_w = $self->_install_max_requests_watcher($self->{endjinn});
