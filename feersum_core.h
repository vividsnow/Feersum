...
/* Kernel-reported unsent/unacked socket send-queue byte count: Linux TIOCOUTQ
 * (alias of SIOCOUTQ, tcp(7)), macOS SO_NWRITE, FreeBSD FIONWRITE.  Without
 * one of these the write deadline keeps its older, simpler behaviour, which is
 * the case on OpenBSD - and on NetBSD, where t/149's two reaping assertions
 * fail with the probe enabled: something there keeps the queue looking like it
 * is draining, so the deadline defers instead of firing.  Not diagnosed, and a
 * write_timeout that never fires is worse than one that is merely strict. */
...
#endif
/* Most we reserve up front for a declared Content-Length body.  The rest is
 * grown as bytes arrive, so a peer cannot turn a header block into a large
 * allocation. */
#ifndef BODY_PREGROW_MAX
# define BODY_PREGROW_MAX (64 * 1024)
#endif
/* Bound one unterminated chunk-size line or trailer line.  RFC 9112 7.1 caps
 * a chunk-size at 16 hex digits; the slack is for chunk extensions.  Nothing
 * else bounds this phase - read_timeout resets on every byte received. */
...
/* First byte of a PP2_TYPE_SSL value is pp2_tlv_ssl.client; this bit is what
 * says the client connected over TLS.  The TLV is also emitted (with the bit
 * clear) just to carry verify/CN data, so its presence alone means nothing. */
...
/* RFC 9110 section 6.4.1 / RFC 7230 section 3.3: 1xx, 204, 205, 304 MUST NOT have a body. */
...
struct feer_req {
    ...
    SV* h2_uri_sv;
#endif
    /* Kept last, excluded from the per-request Zero: only [0, num_headers)
     * is ever read, so stale entries are unreachable. */
...
struct feer_conn {
    ...
    /* HEAD only: the app supplied its own Content-Length, so do not overwrite
     * it with one measured from the (typically emptied) body. */
    unsigned int app_content_length:1;
    /* This request declared a definite body length (Content-Length, including
     * 0, or chunked).  Distinguishes "the body is over" from "no body was ever
     * framed" once the connection reaches RECEIVE_SHUTDOWN - only the former
     * may clamp reads at expected_cl. */
    ...
    /* Consecutive write-deadline checks that saw no send-queue drain.  ACKs
     * can arrive in MSS-sized clumps (64KB on loopback), so one drain-free
     * interval is not proof the peer stopped; three are (conn_write_timeout). */
    unsigned int outq_stalls:2;

    /* Kernel send-queue depth sampled at the last write-deadline check; 0 =
     * no sample yet.  A drop between checks means the peer is still draining
     * (conn_write_timeout); any successful write resets it. */
    int prev_outq;

    /* Same sample for the idle-keepalive reap, kept apart from prev_outq so
     * the two deadlines cannot consume each other's evidence. */
    int prev_read_outq;
    ...
    /* The app closed its tunnel end while ciphertext was still queued: finish
     * flushing, then shut down.  Closing immediately discarded the tail. */
    ...
    /* Set on the PARENT when a stream's write pump stopped on its iteration
     * budget: the write watcher must stay armed to re-enter it, or the
     * response stalls with an open window and an empty buffer. */
    unsigned int           h2_pump_pending:1;
    /* The last stream closed while shutting down, but its response may still
     * be sitting unflushed in tls_wbuf.  Close once the wire has it, not in
     * the nghttp2 callback that discovered it. */
    unsigned int           h2_close_pending:1;
    uint8_t                h2_invalid_frames;
    /* Consecutive peer-silent intervals during which a dispatched stream held
     * response bytes the peer would not take (conn_read_timeout). */
    uint8_t                h2_stall_strikes;
    uint32_t               h2_rst_count;      /* CVE-2023-44487 rapid reset mitigation; uint32 to avoid wrap during a flood */
...
/* feer_listen.pause_flags - independent reasons for stopping accept_w.
 * Watcher is stopped while any bit is set; resumed only when all are clear. */
...
struct feer_server {
    ...
    SV   *shutdown_cb_cv;
    bool  shutting_down;
    /* Retire this worker after max_requests requests.  Checked in C where
     * total_requests is incremented: a Perl-side watcher would either poll
     * (and overshoot badly under load) or run a callback every loop iteration. */
    UV    max_requests;
    SV   *max_requests_cb_cv;
    /* Fired when the limit is reached, before the drain starts: the drain
     * has no deadline, so this is the caller's only chance to bound it. */
    SV   *retire_begin_cb_cv;
    bool  retire_pending;
    /* Called with (method, uri, elapsed_seconds) as each response completes.
     * Lives here rather than in a Perl wrapper around the app: the wrapper ran
     * on every request and, worse, hung the log line off response_guard, which
     * fires when the CONNECTION is released - so under keepalive every entry
    ...
    bool         psgix_io;
    bool         multiprocess;
    /* Graceful shutdown accepts what the kernel already queued on each TCP
     * listener before closing it.  Runner sets this for reuseport workers,
     * whose private queue dies with the fd (clients would see RST). */
    bool         drain_accept_queue;
#ifdef FEERSUM_HAS_H2
    int          max_h2_concurrent_streams;
    /* Cap on undispatched request-body bytes summed across a connection's H2
     * streams.  0 = off (default); the per-stream max_body_len still applies. */
...
#define FEER_REQ_ALLOC(r_) do { \
    ...
    /* headers[] excluded: zeroing 2KB per request was 5% of a PSGI worker */ \
...
#define IOMATRIX_ALLOC(m_) do { \
    ...
    if (iomatrix_freelist != NULL) { \
        ...
        /* iov too: a slot whose count++ happened but whose fill was cut short \
         * by a croak would otherwise writev() the previous connection's \
         * pointer and length straight to the client. */ \
