    ...
    PPCODE:
{
    ...
    if (sni_name) {
        struct feer_sni_entry *e = &lsnr->sni_entries[lsnr->n_sni_entries++];
        STRLEN name_len = strlen(sni_name);
        Newx(e->hostname, name_len + 1, char);
        for (i = 0; (size_t)i < name_len; i++)
            e->hostname[i] = toLOWER(sni_name[i]);
        e->hostname[name_len] = '\0';
        e->ctx_ref = feer_tls_ctx_ref_new(new_ctx);

        trace("SNI entry '%s' added on listener %d (h2=%d)\n",
