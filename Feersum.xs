    ...
    PPCODE:
{
    ...
    if (server->n_listeners == 0) {
         ...
         * fatal once the number has been recycled.  Reachable two ways: a
         * croak out of setup_accept_watcher (epoll_create1 under fd pressure)
         * leaves the slot committed with its fd set, so a retry duplicates it;
         * and so does simply calling use_socket twice with one socket. */
    ...
    PPCODE:
{
    ...
    for (i = 1; i < items; i += 2) {
        ...
        if (strcmp(key, "cert_file") == 0)
            ...
            /* Range-check before narrowing, as max_connections and
             * max_accept_per_loop do: truncating first let 2**32 wrap to 0 and
             * quietly configure TLS on the wrong listener instead of croaking.
             * INT_MIN would also read as "below -1" only by luck. */
            IV want = SvIV(val);
            if (want < -1 || want > INT_MAX)
                croak("set_tls: listener index %" IVdf " out of range (0..%d or -1)",
