...
/* CVE-2019-9512 (HTTP/2 ping flood): past this, stop feeding tls_wbuf and leave
 * frames queued so nghttp2's own outbound-queue flood detection closes the
 * session.  Draining it unconditionally is what kept that detection from firing. */
#define FEER_H2_MAX_WBUF (16 * 1024 * 1024)

/* Silent read_timeout intervals with response bytes still pending before a
 * connection is treated as stalled, not slow.  conn_write_timeout uses three. */
...
struct feer_h2_stream {
    ...
    /* write() calls from poll_cb, INCLUDING zero-length ones: the pump needs
     * to tell "the app wrote nothing" from "the app wrote an empty string",
     * which look identical in the buffer but mean opposite things. */
    unsigned int           writes_seen;
    unsigned int           rst_by_us:1;     /* server initiated the RST; do not count toward CVE-2023-44487 flood */
    unsigned int           body_overflow:1; /* body exceeded max_body_len; reject, do not dispatch */
    int                    limit_status;    /* 414/413: answer with this status instead of dispatching */
};
