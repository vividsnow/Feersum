...
#
# Demonstrates binding to an IPv6 address; this example also enables
# 'reuseport' + pre_fork to show multi-worker shared accept on IPv6.
#
# Usage:
...
my $runner = Feersum::Runner->new(
    listen    => ['[::1]:5000'],  # IPv6 localhost - must be arrayref
    quiet     => 0,
    reuseport => 1,               # IPv6 requires reuseport mode
    pre_fork  => 1,               # reuseport works best with pre_fork
);
